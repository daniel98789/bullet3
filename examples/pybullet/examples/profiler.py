...
class Profiler:
	...
	def profile(self,profile_mask):
		...
		if self.curr_time - self.start_time > self.update_interval:
			...
			if (profile_mask & (1<<0)):
				self.p.addUserDebugText("FPS: " + str((self.frameCounter - self.prev_count)/self.update_interval), [10, 2, -0.4],
						textColorRGB=[0, 0, 0],
						textSize=1.3,
			...
			if (profile_mask & (1<<2)):
				...
				self.p.addUserDebugText("Position: " + "x: " + str(position[0][0]) + " y: " + str(position[0][1]) + " z: " + str(position[0][2]), 
							[10, 0.5, -1.07],
							textColorRGB=[0, 0, 0],
							textSize=1.3,
							lifeTime=self.update_interval+0.1)
			...
			if (profile_mask & (1<<3)):
				self.p.addUserDebugText("Orientation: " "x: " + "{:.2f}".format(position[1][0]) + " y: " + str(position[1][1]) + " z: " + str(position[1][2]), 
							[10, 0, -1.29],
							textColorRGB=[0, 0, 0],
							textSize=1.3,
							lifeTime=self.update_interval+0.1)
